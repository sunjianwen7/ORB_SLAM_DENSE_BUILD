...
public:
	ros::NodeHandle nh;
	ros::Publisher  pub_pointcloud,pub_tcw,pub_camerapath,pub_odom;
	size_t mcounter=0;	 
    ...
    {
	   //创建ROS的发布节点
	   pub_pointcloud= nh.advertise<sensor_msgs::PointCloud2> ("ORBSLAM2/pointcloud", 10); 
	   pub_tcw= nh.advertise<geometry_msgs::PoseStamped> ("ORBSLAM2/CameraPose", 10); 
	   pub_odom= nh.advertise<nav_msgs::Odometry> ("ORBSLAM2/Odometry", 10); 
	   pub_camerapath= nh.advertise<nav_msgs::Path> ("ORBSLAM2/Path", 10); 
    }
...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::PointCloud2> pointcloud_sub(nh,pointcloud_topic, 10);
    ...
    message_filters::Synchronizer<sync_pol> sync(sync_pol(10), rgb_sub,depth_sub,pointcloud_sub);
